...
ggplot(season_summ, aes(season, three_rate, group=1)) +
  ...
  scale_x_continuous(breaks = unique(season_summ$season), labels = as.integer(unique(season_summ$season))) +
  labs(x = "Season", y = "3PA share", title = "Three-Point Attempt Rate Over Time") +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(hjust = 0.5))
...
zone_levels <- c("Paint (non-RA)","Midrange","Corner 3","Above the Break")

non_ra <- selection_long %>%
  filter(bucket != "Restricted Area") %>%
  mutate(season = as.integer(season),
         bucket = factor(bucket, levels = zone_levels)) %>%
  group_by(season) %>%
  mutate(share_nonRA = share / sum(share)) %>%
  ungroup()

ggplot(non_ra, aes(x = factor(season), y = bucket, fill = share_nonRA)) +
  geom_tile() +
  scale_fill_viridis_c(labels = scales::percent, name = "Share of non-RA") +
  labs(x = NULL, y = NULL,
       title = "Non-RA Shot Mix by Zone (Heatmap)") +
  theme_minimal(base_size = 14) +
