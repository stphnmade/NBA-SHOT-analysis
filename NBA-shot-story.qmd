...
# Load data (saved earlier from reticulate as df_r -> RDS)
shots <- readRDS("data/nba_shots.rds") |>
  clean_names()

# --- Derive season label ---
# Prefer SEASON_1/SEASON_2 if present; else compute from GAME_DATE
has_season_cols <- all(c("season_1","season_2") %in% names(shots))
if (has_season_cols) {
  shots <- shots |>
    mutate(season = paste0(season_1, "-", season_2))
} else {
  # Parse GAME_DATE like M-D-Y (e.g., "10-26-2015")
  shots <- shots |>
...
# Normalize categorical fields we’ll use
shots <- shots |>
  mutate(
...
# Long format for stacked area of selection
selection_long <- season_summ |>
  select(season, share_ra, share_paint, share_mid, share_c3, share_atb) |>
